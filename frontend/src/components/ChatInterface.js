...
import { Container, Row, Col, Card, ListGroup, Form, Button, Stack } from "react-bootstrap";
import { get_conversation_by_user_id, get_conversation_title, real_time_chat } from '../services/ChatServices';
...
const ChatInterface = () => {
  ...
  const [currentTopic, setCurrentTopic] = useState(topics.length>0 ? topics[0].conversation_title : []);
...
// handle user send a message to bot.
  const handleUserInputSubmit = (event) => {
    ...
    const botMessage = { message: answerFromBot, sender: "bot" };
  ...
  const handleUserInputTopicSubmit = (event) => {
    ...
    if (!topics.some((topic) => topic.conversation_title === userTopicInput) && !topics.some((topic) => topic.conversation_title === currentTopic)) {
      ...
      <Row className="d-flex justify-content-center" style={{ height: "100%" }}>
        <Col md="10" lg="11" xl="12">
          <Card id="chat1" style={{ borderRadius: "15px", height: "100%" }}>
            ...
            >
              <p className="mb-0 fw-bold">CSE Chatbot</p>
            </Card.Header>
            ...
            <Card.Body>
              <Row style={{ height: "100%" }}>
                      ...
                      {topics.length > 0 ? (
                        topics.map((topic) => (
                          <ListGroup.Item
                            ...
                            onClick={() => handleTopicChange(topic.conversation_title)}
                          >
                ...
                <Col md="8" lg="8" xl="8" style={{ position: "relative" }}>
                  ...
                  <div className="input-section" style={{ position: "absolute", bottom: 0, right: 0, width: "100%", marginTop: "10px" }}>
                    <Stack direction="horizontal" gap={3}>
                      <Form.Control
                        className="me-auto"
                        type="text"
